...
- 按「開始擷取」時若仍有先前結果（包含已匯出結果），以 popup 內對話框詢問是否捨棄並開始；確認後直接開始，取消或關閉對話框保留結果。
- 下載完成後不自動刪除暫存結果。
...
- 可輸出主貼文缺少 post_url 時，展開內容並解析後立即保存該卡片的 outerHTML，置於 JSON 的 missing_post_url_html={encoding:"gzip-base64",data:"…"}。先取得字串再非同步壓縮，保存當時的 DOM；不執行或修改 HTML。有網址者不保存此欄位，舊資料允許缺少。CSV 不追加整段 HTML。
